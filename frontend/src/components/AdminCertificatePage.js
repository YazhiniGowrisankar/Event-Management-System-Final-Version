            ...
            <div className="space-y-3">
              {events.map((event) => (
                <div key={event._id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedEvent?._id === event._id ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-gray-300'}`} onClick={() => handleEventSelect(event)}>
                  ...
                  <p className="text-sm text-gray-600">{formatDate(event.startAt, { fallback: 'No date set' })}</p>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {(certificates || []).map((certificate) => (
                      <tr key={certificate._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-500">{formatDate(certificate.issuedAt, { fallback: '—' })}</div></td>
