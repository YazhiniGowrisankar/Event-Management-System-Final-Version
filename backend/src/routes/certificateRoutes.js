...
router.get("/:eventId/:userId", authMiddleware, async (req, res) => {
  try {
    ...
    gradient.stop(0, "#fdf4ff").stop(1, "#eef2ff");
    doc.rect(0, 0, doc.page.width, doc.page.height).fill(gradient);

    doc.save();
    doc.lineWidth(6).strokeColor("#c4b5fd");
    doc.roundedRect(25, 25, doc.page.width - 50, doc.page.height - 50, 24).stroke();
    doc.restore();

    doc.save();
    doc.fillColor("#f472b6").opacity(0.12).circle(110, 110, 80).fill();
    doc.fillColor("#a5b4fc").circle(doc.page.width - 110, doc.page.height - 140, 90).fill();
    doc.opacity(1);
    doc.restore();
    ...
    doc.fontSize(34).text(titleByType[certificateType], { align: "center", characterSpacing: 1.5 });
    doc.moveDown(0.5);
    doc.fontSize(14).fillColor("#6366f1").text("Presented by Event Management Platform", { align: "center" });
    doc.moveDown(1.5);
    
    doc.fillColor("#1f2937").fontSize(18).text("This certificate proudly recognizes", { align: "center" });
    doc.moveDown(1);
    doc.font("Helvetica-Bold").fontSize(30).text(user.name, { align: "center" });
    doc.moveDown(1);
    ...
    doc.moveDown(1);
    doc.font("Helvetica-Bold").fontSize(22).fillColor("#0f172a").text(`"${event.title}"`, { align: "center" });
    doc.fillColor("#1f2937").font("Helvetica");
    
    if (event.startAt) {
      doc.moveDown(1);
      doc.fontSize(16).text(`Date: ${new Date(event.startAt).toLocaleDateString("en-GB")}`, { align: "center" });
    }
    if (event.location) {
    ...
    // Signature section
    doc.moveDown(2.5);
    doc.lineWidth(1.5).moveTo(doc.page.width / 2 - 120, doc.y + 40).lineTo(doc.page.width / 2 + 120, doc.y + 40).stroke("#e2e8f0");
    try {
      if (fs.existsSync(signaturePath)) {
        doc.image(signaturePath, doc.page.width / 2 - 60, doc.y, { width: 120, align: "center" });
        doc.moveDown(1);
    ...
    doc.moveDown(1.5);
    doc.fontSize(10).fillColor("#64748b").text(`Certificate No: ${certificateNo}`, { align: "center" });
