    ...
    <>
      ...
      {token && role === 'user' && (
        <>
          ...
          <Link to="/calendar" className={`text-sm font-medium px-2 py-1 rounded ${isActive('/calendar') ? 'text-purple-700 bg-purple-50' : 'text-gray-600 hover:text-gray-800 hover:bg-gray-100'}`}>Calendar</Link>
          ...
          <div className="flex items-center gap-3">
            {token && (user?.name || user?.email) && (
              <details className="hidden sm:block relative">
                ...
                <div className="absolute right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow p-3 text-sm text-gray-700 min-w-[200px]">
                  <p className="mb-3"><span className="font-medium">Email:</span> {user.email}</p>
                  <Link to="/profile" className="block w-full text-left px-3 py-2 text-purple-600 hover:bg-purple-50 rounded mb-2">My Profile</Link>
                  <button onClick={onLogout} className="w-full text-left px-3 py-2 text-red-600 hover:bg-red-50 rounded">Logout</button>
                </div>
              </details>
            )}
            {!token ? (
